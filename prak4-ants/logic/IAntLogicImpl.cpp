...
namespace antlogic
{
    char *AntImpl::getMemory() const { return mem; }
    bool AntImpl::hasFood() const { return food; }
    bool AntImpl::isFrozen() const { return frozen; }
    int AntImpl::getFrozenTime() const { return frozen_time; }
    int AntImpl::getTeamId() const { return teamid; }
    antgui::Point &AntImpl::getPoint() const { return p; }
    void AntImpl::setPoint(const antgui::Point &t) const { p = t; }
    void AntImpl::setFrozen(int f_time) { frozen = true; frozen_time = f_time; }
    void AntImpl::setFood(bool _food) { food = _food; }
    void AntImpl::decreaseFrozenTime() {
        if (frozen_time > 0)
            frozen_time--;
        if (frozen_time == 0)
            frozen = false;
